...
class DebateModerator:
    ...
    async def fact_check_statement(
        ...
        """Fact-check a statement using Grok's real-time research capabilities"""
        try:
            # Use Grok's real-time capabilities for comprehensive fact-checking
            fact_check_prompt = f"""
            As a debate moderator with access to real-time information, fact-check this statement:
            
            Statement: "{statement}"
            {f"Speaker: {speaker}" if speaker else ""}
            
            Respond in this format: "✓ Correct: [brief verification]" or "✗ Incorrect: [brief correction]" or "⚠️ Partially true: [clarification]"
            
            Use current, up-to-date information. Keep response under 25 words. Be authoritative but respectful.
            """
            
            # Use context.llm (LiveKit's LLM with Grok) for real-time fact-checking
            response = await context.llm.agenerate(fact_check_prompt)
            fact_check_result = response.choices[0].message.content
            
            # Store the fact-check with Grok verification
            logger.info(f"Grok fact-check completed for speaker: {speaker}")
            
            logger.info(f"Grok fact-check completed for statement: {safe_binary_repr(statement[:50])}...")
            return fact_check_result
            
        except Exception as e:
            logger.error(f"Error in Grok fact-checking: {safe_binary_repr(str(e))}")
            return "Unable to verify that right now - let's continue the discussion."
...
Keep ALL responses under 20 words. Speak with gentle wisdom and focus on harmony."""
        ...
        return """You are Aristotle, the systematic philosopher, moderating a debate with real-time research capabilities via Grok AI.

Your approach:
- ONLY speak when directly called by name or when users ask for your input
- When fact-checking is requested, use Grok's real-time information to provide current, evidence-based responses
- Point out logical fallacies briefly when asked
- Stay quiet and let users debate - only intervene when specifically requested
- Use your fact-checking tool with Grok's real-time capabilities when users ask to verify claims

Keep ALL responses under 20 words. Be logical, structured, and focused on current evidence."""
...
async def entrypoint(ctx: JobContext):
    ...
    try:
        ...
        try:
            ...
            # Setup LLM with xAI Grok (Real-time capabilities for fact-checking)
            try:
                logger.info("🧠 Setting up LLM with xAI Grok...")
                xai_key = os.getenv("XAI_API_KEY")
                if not xai_key:
                    raise ValueError("XAI_API_KEY environment variable is required")
                
                llm = openai.LLM.with_x_ai(
                    model="grok-beta",  # Use Grok's latest model with real-time capabilities
                    api_key=xai_key,
                    temperature=0.3,  # Lower temperature for more factual responses
                )
                logger.info("✅ LLM with xAI Grok configured - real-time fact-checking enabled")
            except Exception as llm_error:
                logger.error(f"❌ LLM setup failed: {type(llm_error).__name__}: {str(llm_error)}")
                logger.error("🔑 Make sure XAI_API_KEY is set in your environment")
                raise llm_error
