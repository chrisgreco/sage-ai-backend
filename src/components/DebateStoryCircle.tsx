  ...
  return (
    <div className="flex-shrink-0 cursor-pointer group">
      <div className="relative">
        {/* Enhanced Story Circle with organic glass effect */}
        <div className={`relative w-20 h-20 bg-gradient-to-br ${room.gradient} p-1 shadow-2xl ${room.isLive ? 'animate-pulse-glow shadow-green-400/30' : 'shadow-black/10'} story-circle`}
             style={{ borderRadius: '50% 45% 55% 50%' }}>
          <div className="w-full h-full backdrop-blur-lg flex items-center justify-center border-2 border-white/60 group-hover:scale-105 transition-all duration-500 shadow-inner icon-container"
               style={{ 
          ...
          {/* Enhanced Live indicator with organic shape */}
          {room.isLive && (
            <div className="absolute -bottom-1 -right-1 w-7 h-7 bg-gradient-to-br from-green-400 to-emerald-500 border-3 border-white flex items-center justify-center shadow-lg shadow-green-500/30 organic-pulse"
                 style={{ borderRadius: '60% 45% 55% 50%' }}>
              <div className="w-2.5 h-2.5 bg-white animate-pulse"
                   style={{ borderRadius: '50% 40% 60% 45%' }}></div>
            </div>
          )}
        </div>
